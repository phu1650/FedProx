...
def share_class(dataset):
    first_site_idx = []
    second_site_idx = []
    for i in range(len(dataset)):
        if dataset[i][1] > 4 : 
            second_site_idx.append(i)
        else:
            first_site_idx.append(i)
    return first_site_idx,second_site_idx
...
class Cifar10Trainer(Executor):
    def __init__(
        ...
        # Training setup
        self.data_path = '/workspace/nvflare/FedProx/Data'
...
#         self.optimizer = SGD(self.model.parameters(), lr=lr, momentum=0.9)
        ...
        self.criterion_prox = PTFedProxLoss()
        # Create Cifar10 dataset for training.
        self.first_site_index = -1
        self.second_site_index = -1 
        
    def setup(self, fl_ctx):
        self.client_name = fl_ctx.get_identity_name()
#         data_path= f"/workspace/nvflare/FedProx/{self.client_name}/data"
        transforms = Compose([ToTensor(),Normalize((0.1307,), (0.3081,)),])
        self.dataset = CIFAR10(root=self.data_path, transform=transforms, download=True, train=True)
        if self.first_site_index == -1 or self.second_site_index:
            self.fist_site_idx,self.second_site_idx = share_class(self.dataset)
        if  self.client_name == 'org1':
            self.dataset = Subset(self.dataset, self.fist_site_idx)
        else:
            self.dataset = Subset(self.dataset, self.second_site_idx)
        indices = np.arange(len(self.dataset))
        self.log_info(fl_ctx, f"len : {len(self.dataset)}")
        y = []
        for i in range(len(self.dataset)):
            y.append(self.dataset[i][1])
        train_indices, val_indices = train_test_split(indices, test_size=0.15, stratify=y,random_state=3407)
    ...
    def local_train(self, fl_ctx, weights, abort_signal):
        ...
        global_model  =  SimpleNetwork()
        global_model.load_state_dict(state_dict=weights)
        global_model.to(self.device)
        # Basic training
        
        for epoch in range(self._epochs):
            train_losses = []
            self.log_info(fl_ctx, f"start training")
            self.model.train()
            correct = 0 
            for i, batch in enumerate(self._train_loader):
                ...
                fed_prox_loss = self.criterion_prox(self.model,global_model )
                cost = self.loss(predictions, labels) + fed_prox_loss
                train_losses.append(cost.item())
                cost.backward()
                self.optimizer.step()
            train_loss = np.mean(train_losses)
            train_acc  = correct / len(self._train_dataset)
#             if self._epochs % 1000 == 0: 
            self.log_info(fl_ctx, f"Epoch: {epoch}/{self._epochs}, Iteration: {i},Accuracy: {train_acc} ,Loss: {train_loss}")
                                   
            self.model.eval()
            val_losses = []
            correct = 0 
            self.log_info(fl_ctx, f"start validating")
            with torch.no_grad():
            ...
            val_acc = correct / len(self._val_dataset)
#             if self._epochs % 1000 == 0: 
            self.log_info(fl_ctx, f"Epoch: {epoch}/{self._epochs}, Iteration: {i},Accuracy: {val_acc} ,Loss: {val_loss}")
