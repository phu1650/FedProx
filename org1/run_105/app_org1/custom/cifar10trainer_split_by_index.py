...
class Cifar10Trainer(Executor):
    def __init__(
        ...
        first_half = list(range(0, len(self._train_dataset)/2))
        second_half = list(range(len(self._train_dataset)/2, len(self._train_dataset)))
        if  self.client_name == 'org1':
            self._train_dataset = Subset(self._train_dataset, first_half)
        else:
            self._train_dataset = Subset(self._train_dataset, first_half)
        indices = np.arange(len(self._train_dataset))
        train_indices, val_indices = train_test_split(indices, test_size=0.15, stratify=self._train_dataset)
        self._train_dataset = Subset(self._train_dataset, train_indices)
        self._val_dataset = Subset((self._train_dataset, val_indices)
